...
async function setUserSubscription(
  ...
  console.log('user from findUserByCustomerId', JSON.stringify(users));
  console.error(`the id is ${users.id} customer ${users.customerId} in setUserSubscription`)
  if (!users.id) {
...
export async function onCheckoutCompleted(
  ...
  console.log('getting customerId from checkout session', customerId);
  ...
  console.log('subscription dada generated by buildSubscription', subscriptionData)
