...
const QrList = ({ qrs, title }: any) => {
  ...
  const pauseQRLink = useCallback((shortLinkId: LinkType) => {
    setLoading(true);
    QrHandler.pauseQRLink(shortLinkId).
      then(() => router.replace("/").then(() => setLoading(false)));
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleDashboard = async () => {
    const dashBaseUrl = process.env.REACT_NODE_ENV === "develop"
  ...
  const renderStaticDynamic = (is: boolean) => (
    <Typography variant="caption" style={{ color: "gray" }}>
      {is ? <SyncIcon fontSize="inherit" /> : <SyncDisabledIcon
        fontSize="inherit" />}
      {is ? " Dynamic" : " Static"}
    </Typography>
  );
                      ...
                      <Box sx={{
                        ...
                        <Box sx={{ display: "flex" }}>
                          <Box sx={{ width: "70px", mx: 1 }}>
                            {!qr.qrOptionsId ||
                              !Object.keys(qr.qrOptionsId).length ? (
                              <Box sx={{ mt: 2, mb: 1.5 }}>
                                <Image src="/ebanuxQr.svg" width={55}
                                  height={55} alt={qr.qrName} />
                              </Box>
                            ) : (
                              <Box sx={{ mt: 1 }}>
                                {renderQr(qr.qrOptionsId,
                                  !qr.isDynamic ? handleDesignerString(
                                    qr.qrType, qr) : qr.qrOptionsId.data, qr)}
                              </Box>
                            )}
                          </Box>
                          <Stack direction="column" sx={{ my: "auto" }}>
                            ...
                            <Typography variant="h6" sx={{
                              fontWeight: "bold",
                              mb: "-2px",
                            }}>{qr.qrName}</Typography>
                            {isWide ? (
                              <Typography variant="caption"
                                sx={{ color: "gray" }}>
                                {`Created at: ${humanDate(
                                  new Date(qr.createdAt).getTime())}`}
                              </Typography>
                            ) : (
                              <Typography variant="caption"
                                sx={{ color: "gray" }}>{/*@ts-ignore*/}
                      ...
                      <Box sx={{ display: "flex" }}>
                        ...
                        <Stack direction="column" spacing={0.8}
                          ...
                          {qrLink.address ? (
                            <Typography variant="caption"
                              sx={{ color: "gray" }}>{/*@ts-ignore*/}
                              <Public fontSize="inherit" /> <Link
                                href={qrLink.link}>{qrLink.link.split(
                                  "//")[1]}</Link>
                            </Typography>) : <></>}
                          <Typography variant="caption" sx={{ color: "gray" }}>
                            <Edit
                              fontSize="inherit" /> {`Updated at: ${humanDate(
                                new Date(qr.updatedAt).getTime())}`}
                          </Typography>
                        </Stack>
                      </Box>
                    </Grid>)}
          ...
          <Grid container justifyContent="center" alignItems="center"
            ...
            <Grid item>
              <Alert severity="info" variant="outlined"
                action={<RenderNewQrButton />} sx={{ width: 450, p: 5 }}>
        ...
        <Dialog
          ...
          <DialogContent>
            <Typography>Are you sure you want to delete the selected
              QR?</Typography>
            <Typography variant="caption" color="orange">This action can not be
              undone.</Typography>
          </DialogContent>
          <DialogActions>
